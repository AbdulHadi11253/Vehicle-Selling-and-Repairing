...
class Userpost extends Database{
    ...
    public function save_image_info(){
        ...
        if (!empty($_FILES['car_image']['tmp_name']) && is_uploaded_file($_FILES['car_image']['tmp_name'])) {
            if ($image_size > 5000000) {
                die('File size is too large! Please upload a smaller file.');
            } elseif (!in_array($ext, $image_extensions_allowed)) {
                die("You must upload a file with one of the following extensions: " . implode(', ', $image_extensions_allowed));
            } else {
                ...
                // Now attempt to move the uploaded file
